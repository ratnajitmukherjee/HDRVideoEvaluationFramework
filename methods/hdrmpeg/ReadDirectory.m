...
%</summary>
    ...
    % Read 1st Frame and estimate the Width and Height of each frame
        ...
        firstFrame = exrread(firstFileName);
    ...
    % Read the frames from the files and store them in the array.
        ...
        parfor i=1:number_of_frames
            num = sprintf('%.5d', (i-1));
            currentFilename = strcat(read_path, num, '.exr');
            hdrFrame = exrread(currentFilename);
            HDR_FRAMES(:,:,:,i) = double(hdrFrame);
        end 
        
        fprintf('\n STAGE 1: Reading EXR file directory complete... \n');
end