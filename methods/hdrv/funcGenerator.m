...
%   Detailed explanation goes here
    ...
    s.pub = bartenPTF(lmin, lmax, bitdepth);
...
function [ pub ] = bartenPTF( lmin, lmax, bitdepth)
%% PU encoding using barten PTF
 ...
 % luminance space 
 x = linspace(0.05, 4000, 2^bitdepth);    
 dL = zeros(size(x));
...
function [puproposed] = proposedPTF(lmin, lmax, bitdepth)
    y = linspace(10^lmin, 10^lmax, 2^bitdepth);
    L_HDR = y;  
    Lout1( L_HDR<1) = 14.9991.*L_HDR(L_HDR<1) - 1e-5;
    Lout1((L_HDR>=1)&(L_HDR<500)) = 39.0358.*(L_HDR((L_HDR>=1)&(L_HDR<500)).^0.4167)-24.0358;
    Lout1( L_HDR>=500) = 405.0632.*log10(L_HDR(L_HDR>=500)) - 597.2547;
...
%     plot(s.pub(:,1), s.pub(:,2), s.pur(:,1), s.pur(:,2), s.puf(:,1), s.puf(:,2), s.pupr(:,1), s.pupr(:,2), s.puvdp(:,1), s.puvdp(:,2), 'LineWidth', 2);
    ...
    PTFs = {'barten', 'hdrv', 'fraunhofer', 'dolby', 'sRGB', 'bbc-hlg', 'proposed'};
